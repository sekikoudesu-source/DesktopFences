from PyQt6.QtWidgets import (
    ...
    QTabWidget, QWidget, QCheckBox, QLabel, QComboBox, QMessageBox
)
...
class SettingsDialog(QDialog):
    def __init__(self, manager):
        ...
        self.setWindowTitle(_("⚙️ Fences 设置面板"))
        ...
        app_layout.addRow(_("🎨 全局透明度:"), op_h_layout)
        ...
        app_layout.addRow(_("⭕ 圆角弧度:"), rad_h_layout)
        

        
        # Language
        self.lang_combo = QComboBox()
        self.lang_combo.addItems([_("中文 (简体)"), _("English")])
        current_lang = self.manager.config.get("language", "zh_CN")
        lang_map = {"zh_CN": 0, "en_US": 1}
        self.lang_combo.setCurrentIndex(lang_map.get(current_lang, 0))
        self.lang_combo.currentIndexChanged.connect(self._on_lang_changed)
        app_layout.addRow(_("🌐 界面语言 (Language):"), self.lang_combo)
        
        self.tabs.addTab(tab_appearance, _("🎨 外观样式"))
        ...
        # 双击桌面显隐
        self.chk_double_click = QCheckBox(_("⚡ 双击桌面空白处隐藏/显示所有收纳盒"))
        ...
        # 移出自动卷帘
        self.chk_rollup = QCheckBox(_("📜 移出鼠标自动卷帘折叠收纳盒 (Roll-up)"))
        ...
        # 防误触锁定
        self.chk_lock = QCheckBox(_("🔒 防误触锁定收纳盒位置与大小 (Lock Position)"))
        ...
        self.tabs.addTab(tab_interactions, _("⚡ 快捷交互"))
    ...
    def _on_lang_changed(self, index):
        lang_keys = ["zh_CN", "en_US"]
        if 0 <= index < len(lang_keys):
            new_lang = lang_keys[index]
            if self.manager.config.get("language") != new_lang:
                self.manager.config["language"] = new_lang
                save_config(self.manager.config)
                QMessageBox.information(self, _("需要重启"), _("语言设置已保存，请重启程序以生效。"))
